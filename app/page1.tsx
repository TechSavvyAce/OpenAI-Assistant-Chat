...
export default function Chat() {
  ...
  const [inputmessage, setInputmessage] = useState('discussion for CRIMINAL RICO');
  ...
  const handleDiscussionAI = async () => {
    console.log('Handling handleDiscussionAI.');
    ...
    while (1) {
      ...
      if (isBot1) {
        ...
        if (threadId1) formData.append('threadId', threadId1);
      }
      formData.append('input', param_query);
      console.log('Sending message to addMessage API endpoint.');
      const addMessageResponse = await fetch('/api/addMessage', {
        method: 'POST',
        body: formData
      ...
      if (isBot1) {
        if (threadId) formData_run.append('threadId', threadId);
      }
      else {

        if (threadId1) formData_run.append('threadId', threadId1);
      }
      const runAssistantResponse = await fetch('/api/runAssistant', {
        method: 'POST',
        body: formData_run
      ...
      let formData_checkStatus = new FormData();
      if (isBot1) {
        if (threadId) formData_checkStatus.append('threadId', threadId);
      }
      else {

        if (threadId1) formData_checkStatus.append('threadId', threadId1);
      }
      if (runAssistantData.runId) {
        formData_checkStatus.append('runId', runAssistantData.runId);
      }
      ...
      if (isBot1) {
        role = "assistant";
        isBot1 = false;
        if (threadId) formData_listMessage.append('threadId', threadId);
      }
      else {
        role = "user";
        isBot1 = true;
        if (threadId1) formData_listMessage.append('threadId', threadId1);
      }
      ...
      if (listMessagesResponse.ok) {
        const lastMessage = listMessagesData.messages[0];
        console.log('Adding assistant\'s message to the chat.');
        param_query = lastMessage.content[0].text.value;
        setChatMessages(prevMessages => [
          ...prevMessages,
          { role: role, content: param_query }
        ]);
      } else {
        console.error('Error retrieving messages:', listMessagesData.error);
        break;
  ...
  useEffect(() => {
    if (assistantId && threadId && threadId1 && inputmessage != '') {
      //handleDiscussionAI();
    }

  }, [assistantId, threadId, threadId1, inputmessage])
        ...
        <div className="border-gray-500 bg-gray-200 sm:mx-0 mx-5 mt-20 max-w-screen-md rounded-md border-2 sm:w-full">
          <div className="flex flex-col space-y-4 p-7 sm:p-10">
            ...
            >
              ...
              >
                {isStartLoading ? <LoadingCircle /> : "Start Discussion"}
              </button>
            </form>
          </div>
        </div>
